...
| D-62 | A Go method called on its own receiver resolves inside its package, because the receiver word is the one the enclosing method binds and Go keeps every method of a type in one package | done | src/graphrag/extract.py, src/graphrag/receivers.py, src/graphrag/store.py, src/graphrag/config.py, src/graphrag/indexwrite.py, src/graphrag/dbread.py, src/graphrag/resolve.py, src/graphrag/resolvedb.py, src/graphrag/query.py, tests/test_resolve.py, tests/test_index.py | T-348..T-350 |
| D-63 | The SCIP overlay is auto by language: a project's `scip` moves from `bool` to `bool or None`, `None` runs the tier for an installed indexer needing no build the tree cannot resolve, and only an explicit `false` opts out | done | src/graphrag/projcfg.py, src/graphrag/scip/__init__.py, src/graphrag/index.py, src/graphrag/config.py, tests/test_projcfg.py, tests/test_scip_auto.py | T-351..T-359 |
| D-64 | The watch arms in a child process, because watchfiles' `RustNotify` holds the GIL for the whole arming walk and in-process that starved the watchdog pinger | done | src/graphrag/watch.py, src/graphrag/watchchild.py, tests/test_watch.py | T-360 |
| D-65 | `find_symbol`, `neighbors` and `blast_radius` carry `_meta.anthropic/alwaysLoad`, so a Claude session can call them on its first turn with no `ToolSearch` round trip; `index` stays deferred. Split `_rows`/`_answer` into `answers.py` to hold `tools.py` under the line ceiling | done | src/graphrag/tools.py, src/graphrag/answers.py, tests/test_tools.py, tests/test_hygiene.py | T-361 |
