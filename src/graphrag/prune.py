...
from . import config, federation, progress, quarantine, registry
...
def _is_idle(store: Path) -> bool:
    """Nothing has written the graph for `PRUNE_MIN_IDLE_S`.

    The semantic engine has a defect recorded for skipping this -- a prune raced
    a store the daemon was mid-write on. The knob existed here since the rebuild
    and was read by nothing, so this is the floor being built rather than
    switched on.
    """
    try:
...
def _retire_stores(keys: list[str]) -> list[str]:
    """Move each dead row's graph into quarantine. Returns the ones that moved.

    A row leaving used to free no bytes at all: the watcher removed the row and
    the graph stayed on disk until a human typed a prune. This is that half,
    and it moves rather than deletes -- see `quarantine`.
    """
    moved: list[str] = []
    for key in keys:
