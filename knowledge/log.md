...
- **Update**: [the graph answers the caller question](computations/the-graph-answers-the-caller-question.md) - the lexical and semantic arms read 0.412 and 0.312 from a run whose coderag daemon was down and whose empty result sets scored zero. A serialized run on a committed tree gives 0.535 and 0.331, and the graph returns to F1 1.000 once the TRUTH row for `capabilities` names the caller `extract.py` gained.
