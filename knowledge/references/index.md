...
* [The coderag CLI holds its own GPU session, and the card is
  ...
  `coderag search` twenty times, and every invocation built its own CUDA session beside the
  daemon's. The card is 16303 MiB, the daemon holds 7660 MiB and one CLI search added 3114 MiB, so
  a third consumer exhausted it. `coderag` at `dd0fec0` makes the CLI ask the daemon instead.
